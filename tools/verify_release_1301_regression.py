...
allowed_new = {Path('app/src/main/res/drawable/ic_expand_more.xml')}
...
for new in sorted((root / 'app/src/main').rglob('*')):
    ...
    if not (pred / rel).exists() and rel not in allowed_new:
...
if digest(root / 'app/build.gradle.kts') == digest(pred / 'app/build.gradle.kts'):
    errors.append('release identity file did not change')
...
print(f'PASS VulkanScope 1.3.0 -> 1.3.1 runtime regression boundary; lockedRuntimeFiles={locked}, newRuntimeFiles=1')
