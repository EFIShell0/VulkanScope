...
need('ScrollBoundaryIndicators(listState, Modifier.fillMaxSize().padding(horizontal = 12.dp, vertical = 10.dp))' in page,'top-level overlay indicator placement missing')
...
for token in ['Dialog(onDismissRequest = onDismiss, properties = DialogProperties(usePlatformDefaultWidth = false))','widthIn(max = 560.dp)','shape = MaterialTheme.shapes.extraLarge','horizontalMargin = if (configuration.screenWidthDp < 360) 10.dp else 18.dp','verticalMargin = if (configuration.screenHeightDp < 520) 8.dp else 16.dp','bodyMaxHeight = minOf(540.dp, maxOf(140.dp, (configuration.screenHeightDp - 200).dp))','CompositionLocalProvider(LocalDetailKeyValuePresentation provides true)','ScrollBoundaryIndicators(scrollState, Modifier.fillMaxSize().padding(horizontal = 8.dp, vertical = 6.dp))','ExpressiveTextButton("Close")']:
...
need('ScrollBoundaryIndicators(listState, Modifier.fillMaxSize().padding(horizontal = 8.dp, vertical = 6.dp))' in release,'release-note overlay indicator missing')
