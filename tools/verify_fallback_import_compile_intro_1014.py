...
vm=re.search(r'versionName\s*=\s*"1\.0\.(\d+)"', gradle)
release_minor=int(vm.group(1)) if vm is not None else 0
if not a.skip_version:
    vm=re.search(r'versionName\s*=\s*"1\.0\.(\d+)"', gradle); cm=re.search(r'versionCode\s*=\s*(\d+)', gradle)
    need(vm is not None and cm is not None and int(vm.group(1)) >= 14 and int(cm.group(1)) >= 1014, 'retained 1.0.14+ release identity missing')
...
expected_source_icon = 'R.drawable.ic_zip_download' if release_minor >= 17 else 'R.drawable.ic_action_github'
