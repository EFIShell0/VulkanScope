...
def main():
    ...
    root=Path(a.root).resolve(); pred=Path(a.predecessor).resolve(); c=json.loads((root/'tests/golden/1.3.12_filter_driver_branding_contract.json').read_text())
    ...
    norm=cb.replace('versionCode = 1313','versionCode = 1312').replace('versionName = "1.3.13"','versionName = "1.3.12"')
    if norm!=pb: raise AssertionError('build.gradle changed beyond release identity')
    print('release_1313 regression boundary: PASS')
if __name__=='__main__':
    try: main()
    except Exception as e: print('release_1313 regression boundary: FAIL:',e,file=sys.stderr); sys.exit(1)
