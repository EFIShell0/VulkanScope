...
def main():
    ...
    root=Path(a.root).resolve(); pred=Path(a.predecessor).resolve(); c=json.loads((root/'tests/golden/1.3.11_filter_storage_driver_contract.json').read_text())
    ...
    norm=cb.replace('versionCode = 1312','versionCode = 1311').replace('versionName = "1.3.12"','versionName = "1.3.11"')
    if norm!=pb: raise AssertionError('build.gradle changed beyond release identity')
    print('release_1312 regression boundary: PASS')
if __name__=='__main__':
    try: main()
    except Exception as e: print('release_1312 regression boundary: FAIL:',e,file=sys.stderr); sys.exit(1)
