...
if not registry_path.is_file():
    raise SystemExit('canonical current Vulkan vk.xml input is missing')
...
for node in registry.findall('./types/type'):
    if (node.findtext('name') or node.get('name')) == 'VK_HEADER_VERSION' and 'vulkan' in {x.strip() for x in (node.get('api') or '').split(',')}:
        ...
        if match:
            ...
            break
if header_version is None or header_version < 361:
...
if extension is None:
    ...
    if 'VkPhysicalDevicePrivateDataBaseHandleFeaturesNV' not in required_types:
        errors.append('1.4.362 private-data-base-handle feature struct is missing from extension requirements')
    if ('VkPhysicalDevicePrivateDataBaseHandleFeaturesNV', 'privateDataBaseHandle') not in required_features:
        errors.append('1.4.362 privateDataBaseHandle feature member is missing from extension requirements')
...
if not lock_path.is_file():
    ...
    if lock.get('apiBaseline') != 'Vulkan 1.4.362': errors.append('current registry lock baseline is not Vulkan 1.4.362')
    if lock.get('headerVersion') != 362: errors.append('current registry lock header version is not 362')
    if lock.get('registrySha256') != expected_sha: errors.append('registry lock SHA does not match canonical 1.4.362 input')
...
if cpp.count('vulkanRegistryVersion\\\":\\\"1.4.362') < 3:
    errors.append('native complete-report provenance is not fully pinned to current Vulkan 1.4.362')
...
if errors:
    for error in errors:
        print('FAIL:', error)
    raise SystemExit(1)
print('PASS retained Vulkan 1.4.362 regressions on current 1.4.362 baseline: private-data-base-handle query/report coverage, canonical registry census, bounded direct publication, dedicated-process semantics')
