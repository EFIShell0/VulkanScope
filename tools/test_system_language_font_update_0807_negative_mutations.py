...
mutate_file('blue update icon removal', main_rel, 'is UpdateStatus.Available -> { UpdateInfoIcon();', 'is UpdateStatus.Available -> {')
mutate_file('update icon color regression', main_rel, 'tint = ComposeColor(0xFF5CA9FF)', 'tint = VulkanAccentSoft')
mutate_file('old UPDATE badge restoration', main_rel, 'is UpdateStatus.Available -> { UpdateInfoIcon();', 'is UpdateStatus.Available -> { UpdateStatusBadge("UPDATE"); UpdateInfoIcon();')
