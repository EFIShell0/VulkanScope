# legacy reference parity methodology

This is a source-level audit only. It never infers runtime support from the database or from device model names.

When a local legacy reference 4.12 checkout is available, run `tools/compare_reference_4_12.py` or `tools/audit_extension_reference.py --mode source` to classify source-referenced physical-device structs and fields as missing or partial.
