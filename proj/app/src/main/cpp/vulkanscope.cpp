...
std::string collect(jobject surfaceObject, JNIEnv* env, const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir) {
    ...
    const bool hasLiveSurface = surfaceObject && env && surfaceExtensionAvailable && androidSurfaceExtensionAvailable;
    ...
    std::ostringstream out;
    out << "{\"status\":\"available\",\"reason\":\"\",\"loaderVersion\":" << jsonString(versionString(loaderVersion));
    ...
    for (uint32_t deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
        ...
        VkPhysicalDeviceProperties properties{};
        g_probeStage = 6;
        __android_log_print(ANDROID_LOG_INFO, "VulkanProbe", "base device[%u] properties begin", deviceIndex);
        const bool propertiesOk = getDevicePropertiesStable(api, device, properties);
        __android_log_print(ANDROID_LOG_INFO, "VulkanProbe", "base device[%u] properties end ok=%s name=%s api=%s", deviceIndex, propertiesOk ? "true" : "false", properties.deviceName, versionString(properties.apiVersion).c_str());
        if (!propertiesOk) {
            api.destroyInstance(instance, nullptr);
        ...
        const uint32_t maxVersionedMinor = std::min({4u, VK_API_VERSION_MINOR(apiVersion), VK_API_VERSION_MINOR(instanceApiVersion)});
        for (uint32_t targetMinor = 1; targetMinor <= maxVersionedMinor; ++targetMinor) {
        ...
        out << ']';
        const bool vulkan14Available = coreExtendedQueriesAvailable && VK_API_VERSION_MINOR(apiVersion) >= 4 && VK_API_VERSION_MINOR(instanceApiVersion) >= 4;
    ...
    out << "]}";
    if (liveSurface != VK_NULL_HANDLE && api.destroySurfaceKHR) api.destroySurfaceKHR(instance, liveSurface, nullptr);
    api.destroyInstance(instance, nullptr);
    return out.str();
}
