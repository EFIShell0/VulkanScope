...
std::string layersJson(VulkanApi& api, const std::vector<VkLayerProperties>& values) {
    ...
    for (size_t i = 0; i < values.size(); ++i) {
        ...
        out << "{\"name\":" << jsonString(values[i].layerName)
            ...
            << ",\"extensions\":";
        uint32_t extCount = 0;
        VkResult extResult = api.enumerateInstanceExtensionProperties ? api.enumerateInstanceExtensionProperties(values[i].layerName, &extCount, nullptr) : VK_ERROR_EXTENSION_NOT_PRESENT;
        if (extResult != VK_SUCCESS || extCount == 0 || extCount > kMaxExtensionEntries) {
            out << "[]}";
            continue;
        }
        std::vector<VkExtensionProperties> extensions(extCount);
        extResult = api.enumerateInstanceExtensionProperties(values[i].layerName, &extCount, extensions.data());
...
std::vector<const char*> buildQueryInstanceExtensions(VulkanApi& api) {
    ...
    if (api.enumerateInstanceVersion) {
        if (api.enumerateInstanceVersion(&loaderVersion) != VK_SUCCESS) loaderVersion = VK_API_VERSION_1_0;
    }
    if (VK_API_VERSION_MINOR(loaderVersion) < 1) {
        const auto instanceExts = instanceExtensions(api);
        if (hasExtension(instanceExts, "VK_KHR_get_physical_device_properties2")) result.push_back("VK_KHR_get_physical_device_properties2");
    }
    return result;
}
...
static const char* generatedCanonicalEnumName(const char* type, int64_t value);
static std::string generatedCanonicalFlagsName(const char* type, uint64_t value);
...
void generatedEmitUnsupported(std::vector<GeneratedField>& dst, const char* section, const char* name, const char* type) {
    dst.push_back({false, section, name, std::string("unavailable; unsupported pointer field type=") + (type ? type : "unknown")});
}
...
void generatedEmitEnum(std::vector<GeneratedField>& dst, const char* section, const char* name, const char* type, int64_t value) {
    const char* canonical = generatedCanonicalEnumName(type, value);
    std::ostringstream ss;
    if (canonical) ss << canonical << " = ";
    ss << value << " (0x" << std::uppercase << std::hex << static_cast<uint64_t>(value) << ")";
    dst.push_back({false, section, name, ss.str()});
}
void generatedEmitFlags(std::vector<GeneratedField>& dst, const char* section, const char* name, const char* type, uint64_t value) {
    const std::string canonical = generatedCanonicalFlagsName(type, value);
    std::ostringstream ss;
    if (!canonical.empty()) ss << canonical << " = ";
    ss << "0x" << std::uppercase << std::hex << value;
    dst.push_back({false, section, name, ss.str()});
}
template <typename T> void generatedEmitEnumArray(std::vector<GeneratedField>& dst, const char* section, const char* name, const T* values, size_t count, const char* type) {
    std::string joined;
    for (size_t i = 0; i < count; ++i) {
        if (i) joined += ", ";
        const char* canonical = generatedCanonicalEnumName(type, static_cast<int64_t>(values[i]));
        std::ostringstream ss;
        if (canonical) ss << canonical << " = ";
        ss << static_cast<int64_t>(values[i]) << " (0x" << std::uppercase << std::hex << static_cast<uint64_t>(values[i]) << ")";
        joined += ss.str();
    }
    dst.push_back({false, section, name, joined});
}
template <typename T> void generatedEmitFlagsArray(std::vector<GeneratedField>& dst, const char* section, const char* name, const T* values, size_t count, const char* type) {
    std::string joined;
    for (size_t i = 0; i < count; ++i) {
        if (i) joined += ", ";
        const std::string canonical = generatedCanonicalFlagsName(type, static_cast<uint64_t>(values[i]));
        std::ostringstream ss;
        if (!canonical.empty()) ss << canonical << " = ";
        ss << "0x" << std::uppercase << std::hex << static_cast<uint64_t>(values[i]);
        joined += ss.str();
    }
    dst.push_back({false, section, name, joined});
}
void generatedEmitFloat(std::vector<GeneratedField>& dst, const char* section, const char* name, float value) {
    dst.push_back({false, section, name, std::to_string(value)});
}
template <typename T> void generatedEmitFloat(std::vector<GeneratedField>& dst, const char* section, const char* name, T value) {
    dst.push_back({false, section, name, std::to_string(value)});
}
...
std::string collect(jobject surfaceObject, JNIEnv* env, const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir) {
    ...
    if (deviceCount > kMaxPhysicalDeviceEntries) {
        api.destroyInstance(instance, nullptr);
    ...
    for (uint32_t deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
        ...
        out << "],\"heapSafetyRejected\":" << jsonBool(memoryHeapSafetyRejected)
            << ",\"typeSafetyRejected\":" << jsonBool(memoryTypeSafetyRejected) << "},\"formats\":[";
...
std::string collectVulkanCoreGroup(const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir, uint32_t targetMinor) {
    ...
    VkInstance instance = nullptr;
    const VkResult createResult = api.createInstance(&createInfo, nullptr, &instance);
    ...
    if (count > kMaxPhysicalDeviceEntries) { api.destroyInstance(instance, nullptr); return std::string("{\"status\":\"unavailable\",\"reason\":\"Physical-device count exceeds safety limit.\",\"devices\":[]}"); }
...
std::string collectVulkanExtensionGroup(const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir, const char* group) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        if (std::strcmp(groupName, "videoCapabilities") == 0) {
            ...
            VkPhysicalDeviceFeatures2 features2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, &features, {}};
            api.queryFeatures2(devices[i], &features2);
            addFeature("shaderOcpMicroscalingTypes", features.shaderOcpMicroscalingTypes);
...
std::string collectVulkanAdvancedGroup(const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir, const char* group) {
    ...
    if (count > kMaxPhysicalDeviceEntries) {
        api.destroyInstance(instance, nullptr);
        return std::string("{\"status\":\"unavailable\",\"group\":") + jsonString(group ? group : "") + ",\"reason\":\"Physical-device count exceeds safety limit.\",\"devices\":[]}";
    }
    if (countResult != VK_SUCCESS || count == 0) {
        api.destroyInstance(instance, nullptr);
            ...
            if (toolCount > 0) {
                ...
                api.getPhysicalDeviceToolProperties(devices[i], &toolCount, tools.data());
                {
                    ...
                    for (uint32_t t = 0; t < toolCount; ++t) {
                        ...
                        out << "{\"section\":\"Vulkan Tool\",\"name\":" << jsonString(tools[t].name) << ",\"value\":" << jsonString(std::string(tools[t].description) + " | version " + std::to_string(tools[t].version) + " | purposes=" + toolPurposes(tools[t].purposes) + " | layer=" + tools[t].layer) << '}';
                    }
                    out << "]";
                }
            }
            ...
            if (gc && api.enumeratePhysicalDeviceGroups(instance, &gc, groups.data()) == VK_SUCCESS) {
                ...
                for (uint32_t g = 0; g < gc; ++g) {
                    ...
                    for (uint32_t d = 0; d < groups[g].physicalDeviceCount && d < VK_MAX_DEVICE_GROUP_SIZE; ++d) {
                        if (d) groupValue << "; ";
                        std::array<uint8_t, 256> gpRaw{};
                        api.getPhysicalDeviceProperties(groups[g].physicalDevices[d], gpRaw.data());
                        const uint32_t gpVendor = readU32Unaligned(gpRaw.data() + 8);
                        const uint32_t gpDevice = readU32Unaligned(gpRaw.data() + 12);
                        const char* gpName = reinterpret_cast<const char*>(gpRaw.data() + 20);
...
std::string collectVulkanSimpleFeatureGroup(const char* driverMode, const char* driverIcdPath, const char* driverBundlePath, const char* hookLibDir, const char* group) {
    ...
    uint32_t count=0; const VkResult countResult = api.enumeratePhysicalDevices(inst,&count,nullptr); if(count > kMaxPhysicalDeviceEntries){api.destroyInstance(inst,nullptr);return std::string("{\"status\":\"unavailable\",\"group\":")+jsonString(group)+",\"extension\":"+jsonString(extensionName)+",\"reason\":\"Physical-device count exceeds safety limit.\",\"devices\":[]}";} if(countResult!=VK_SUCCESS || !count){api.destroyInstance(inst,nullptr);return std::string("{\"status\":\"not_applicable\",\"group\":")+jsonString(group)+",\"extension\":"+jsonString(extensionName)+",\"reason\":\"No physical devices.\",\"devices\":[]}";} std::vector<VkPhysicalDevice> devs(count); if(api.enumeratePhysicalDevices(inst,&count,devs.data())!=VK_SUCCESS){api.destroyInstance(inst,nullptr);return std::string("{\"status\":\"unavailable\",\"group\":")+jsonString(group)+",\"extension\":"+jsonString(extensionName)+",\"reason\":\"Physical device enumeration failed.\",\"devices\":[]}";}
    ...
    for(uint32_t i=0;i<count;++i){auto exts = deviceExtensions(api, devs[i]); bool supported = std::any_of(exts.begin(), exts.end(), [&](const VkExtensionProperties& e){ return std::strcmp(e.extensionName, extensionName) == 0; }); if (!supported) continue; std::array<uint8_t,1024> raw{}; api.getPhysicalDeviceProperties(devs[i],raw.data()); uint32_t vendor=0, deviceId=0; std::memcpy(&vendor, raw.data()+8, sizeof(vendor)); std::memcpy(&deviceId, raw.data()+12, sizeof(deviceId)); const char* name=reinterpret_cast<const char*>(raw.data()+20); if(first)out<<',';first=true; out<<"{\"vendorId\":"<<vendor<<",\"deviceId\":"<<deviceId<<",\"name\":"<<jsonString(name?name:"Unknown GPU")<<",\"features\":[";
        if(std::strcmp(group,"maintenance11")==0){VkPhysicalDeviceMaintenance11FeaturesKHR f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_KHR_maintenance11 / maintenance11\",\"supported\":"<<jsonBool(f.maintenance11==VK_TRUE)<<"}";}
        else if(std::strcmp(group,"deviceAddressCommands")==0){VkPhysicalDeviceDeviceAddressCommandsFeaturesKHR f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_KHR_device_address_commands / deviceAddressCommands\",\"supported\":"<<jsonBool(f.deviceAddressCommands==VK_TRUE)<<"}";}
        else if(std::strcmp(group,"shaderUniformBufferUnsizedArray")==0){VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_EXT_shader_uniform_buffer_unsized_array / shaderUniformBufferUnsizedArray\",\"supported\":"<<jsonBool(f.shaderUniformBufferUnsizedArray==VK_TRUE)<<"}";}
        else if(std::strcmp(group,"dataGraphOpticalFlow")==0){VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_ARM_data_graph_optical_flow / dataGraphOpticalFlow\",\"supported\":"<<jsonBool(f.dataGraphOpticalFlow==VK_TRUE)<<"}";}
        else if(std::strcmp(group,"pipelineCacheIncrementalMode")==0){VkPhysicalDevicePipelineCacheIncrementalModeFeaturesSEC f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_SEC_pipeline_cache_incremental_mode / pipelineCacheIncrementalMode\",\"supported\":"<<jsonBool(f.pipelineCacheIncrementalMode==VK_TRUE)<<"}";}
        ...
        else {VkPhysicalDeviceShaderOcpMicroscalingTypesFeaturesEXT f{structureType,nullptr,VK_FALSE};VkPhysicalDeviceFeatures2 f2{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,&f,{}};api.queryFeatures2(devs[i],&f2);out<<"{\"name\":\"VK_EXT_shader_ocp_microscaling_types / shaderOcpMicroscalingTypes\",\"supported\":"<<jsonBool(f.shaderOcpMicroscalingTypes==VK_TRUE)<<"}";}
        out<<"]}";
    }
